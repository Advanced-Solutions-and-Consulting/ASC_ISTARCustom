...
using RestSharp;
using AcumaticaMetalsAPI.Infrastructure;
...
{
    [Obfuscation(Feature = "encryptmethod", Exclude = true, ApplyToMembers = true)]
    public static class MetalsAPIConstants
    {
        public const string DefaultApiUrl = "https://metals-api.com/";
    }
    [Obfuscation(Feature = "encryptmethod", Exclude = true, ApplyToMembers = true)]
    public class MetalsApi
    {
        ...
        public MetalsApi(string accessKey, object parameters)
        {
            apiUrl = MetalsAPIConstants.DefaultApiUrl + "/" ;


            PXTrace.WriteInformation($"Calling {apiUrl}");
            ...
            {
                throw new ArgumentException("Please provide a Metals API Access Key", nameof(accessKey));
            }
        ...
        protected virtual T SendRequest<T>(string endpoint, object body = null, Method httpMethod = Method.POST) where T : new()
        {
            ...
            {
                var metalsError = JsonConvert.DeserializeObject<MetalsApiError>(response.Content);
                var errorMessage = metalsError.error.code + " - " + metalsError.error.type;
                throw new MetalsApiException(response.StatusCode, metalsError, errorMessage);
            }
        ...
        protected virtual async Task<T> SendRequestAsync<T>(string endpoint, object body = null, Method httpMethod = Method.POST) where T : new()
        {
            ...
            if ((int)response.StatusCode >= 400)
            {
                var metalsApiError = JsonConvert.DeserializeObject<MetalsApiError>(response.Content);
                var errorMessage = metalsApiError.error.code + " - " + metalsApiError.error.info;
                throw new MetalsApiException(response.StatusCode, metalsApiError, errorMessage);
            }
        ...
        public virtual RateResponse GetExchangeRates()
        {
            var response = SendRequest<RateResponse>("categories", null, Method.GET);
            return response;
        }
